...
export function* createDialog(action = {}) {
  ...
  try {
    console.log("Create Dialog Request Data ==>", data)   
    
    const dialog = yield call(QB.chat.createDialog, data)
    dialog.color = getRandomColor()

    console.log("Created Dialog is ==>", dialog)  
...
export function* joinDialog(action = {}) {
  ...
  try {
    ...
    //   if (dialog2 && dialog2.type === QB.chat.DIALOG_TYPE.GROUP_CHAT) {
    //     const myName = user.fullName || user.login || user.email
    //     const body = `${myName} joined into the dialog`
    ...
    const dialog = yield call(QB.chat.joinDialog, data)
    const result = dialogJoinSuccess(dialog)
    yield put(result)  
    yield call(notifySystemMessage, dialog.id, '1')      
    if (resolve) resolve(result)
  } catch(e) {
    // yield put(dialogJoinFail(e.message))
    const result = dialogJoinFail(e.message)
    yield put(result)
    if (reject) reject(result)
  }
}
